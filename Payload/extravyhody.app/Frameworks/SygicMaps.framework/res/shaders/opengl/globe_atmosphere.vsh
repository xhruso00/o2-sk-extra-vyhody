...
void main(void)
{
    ...
    vec3 position = a_vPosition0.xyz;
    vec3 camPos = hu_vCameraPos;
    ...
    float altitude = camHeight - u_fGlobeRadius;
    float horizonDist = sqrt((altitude*altitude) + (2.0*u_fGlobeRadius*altitude));
    float maxDot = horizonDist / camHeight;
 
    // get distance to atmosphere horizon - use max(0,...) because we can go into the atmosphere
    altitude = max(0.0, camHeight - u_fAtmosphereRadius);
    horizonDist = sqrt((altitude*altitude) + (2.0*u_fAtmosphereRadius*altitude));
    ...
    //float minDot2 = ((camHeight - u_fGlobeRadius) * (1.0 / (u_fAtmosphereRadius  - u_fGlobeRadius))) - (1.0 - tweakAmount);
    ...
    //v_vTex0UV.x = brightness * saturate(lc + 1.0 + StretchAmt);
    //v_vTex0UV.y = height;
